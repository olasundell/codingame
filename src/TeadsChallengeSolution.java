	...
	public static String solve(Scanner in) {
        ...
        for (int i = 0; i < n; i++) {
	        ...
	        if (!relations.containsKey(xi)) {
		        relations.put(xi, new ArrayList<>());
	        }

	        relations.get(xi).add(yi);

	        if (!reverseRelations.containsKey(yi)) {
		        reverseRelations.put(yi, new ArrayList<>());
	        }
...
//
//        return String.valueOf(findLeastDistance(nodeMap)); // The minimal amount of steps required to completely propagate the advertisement
//    }
//
